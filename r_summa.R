...
predict.summa <- function(summa, newdata = NULL, Y=NULL, alpha=1.0, newmodellist = NULL, 
  ...
  summa$lambdalist <- map(summa$roclist, lambda_fromROC, N=length(summa$Y), rho=summa$rho)
...
summary.summa <- function(summa, full=TRUE) {
  ...
  l1 <- map_dbl(summa$lambdalist, 1)
  l2 <- map_dbl(summa$lambdalist, 2)
  rs <- map_dbl(summa$lambdalist, 3)
  
  # prepare data.frame
  res <- tibble(ROC=ROC, l1=l1, l2=l2, rs=rs, model=names(summa$roclist), 
